...
# Define the file path for pp-2022.csv
file_path <- "D:/ST5014CEM-Data-Science-For-Developers/1.0 Obtain Data/1.0 Housing Prices/pp-2022.csv"

# Function to verify column count and convert Date format for pp-2022.csv
verify_and_process_pp_2022 <- function(file_path) {
  # Read the CSV file
  ...
  # Convert Date column to POSIXct format if needed ("6/6/2022 0:00" format)
  data$Date <- as.POSIXct(data$Date, format = "%m/%d/%Y %H:%M", tz = "UTC")
  
  # Convert Date column to ISO8601 format ("2022-06-06T00:00:00Z" format)
...
# Call the function to verify and process pp-2022.csv
processed_data <- verify_and_process_pp_2022(file_path)

# Show column names and first 10 rows to verify
cat("Column names of pp-2022.csv after processing:\n")
print(colnames(processed_data))
cat("\n")

cat("First 10 rows of pp-2022.csv after processing:\n")
print(head(processed_data, 10))

# Save the processed data back to pp-2022.csv
write_csv(processed_data, file_path, col_names = TRUE)

# Confirmation message
cat("Processed data has been saved to pp-2022.csv successfully.\n")
